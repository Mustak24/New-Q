...
export function RoundButton({tailwindcss='', text='', onClick=()=>{}, innerHTML='Click'}) {
...
export function LongWidthBnt({ innerHTML = 'Click', isLoading=false, loadingInnerHTML = 'Wait ...', tailwindcss = '', bg = 'white', text = 'black', icon = <TbCubeSend /> }) {
    return (<button className={`${tailwindcss} flex items-center justify-center overflow-hidden relative group min-h-10 px-5 rounded-full font-bold font-sans border-2 border-[var(--text)] hover:bg-[var(--text)] text-[var(--text)] hover:text-[var(--bg)] transition-all duration-300 `}
