...
export function SpinLoader({priColor='black', secColor='white', size='20px', border='6px', tailwindcss, isHover=false}) {
    return (<div 
        className={`${tailwindcss} rounded-full animate-spin size-[var(--size)]`}
        style={{
          '--size': size, 
          borderWidth: border,
          borderColor: isHover ? 'transparent' : priColor,
          borderTopColor: isHover ? secColor : 'transparent',
        }}
      ></div>)
}